...
export async function getProgramasByEstacion(
  estacionId: number
): Promise<{
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/programa-mantenimiento/estacion/${estacionId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
        credentials: "include", // 🔐 usa cookie/token
      }
    );
